# 📄 초급과정 A반 공지사항
<p align='center'>
  <a href="https://github.com/JSeong2024/2025-MYPAUL-PYTHONEDU/tree/main/PYTHON-2025-09/Basic/Class-A">
    <img src="https://img.shields.io/badge/강의실으로%20돌아가기-F3EC69?&style=for-the-badge&&logoColor=white"/>
  </a>
</p>

---

## 2025.09.28. 일요일
- 파이썬 기초 문법을 간단하게 정리한 '**파이썬 가이드**'를 만들었습니다. 해당 자료를 참고하기 바랍니다. [click here](https://github.com/JSeong2024/2025-MYPAUL-PYTHONEDU/tree/main/PYTHON-2025-09/MoreStudy/PythonGuide)

## 2025.09.26. 금요일
- 개인 코딩 시간이 있도록 하겠습니다. (오늘은 1주차 내용을 복습하는데 시간을 많이 써서 각자 코딩 연습할 시간이 없었지만, 3주차부터는 반드시 있도록 하겠습니다.)
- 수업 시간에 반드시 한명 한명 피드백이 이루어지도록 할 예정입니다.
- [2주차 과제가 업로드 되었습니다.](https://github.com/JSeong2024/2025-MYPAUL-PYTHONEDU/tree/main/PYTHON-2025-09/Basic/Class-A/Assignment)
- [2주차 내용 정리본을 업로드 하였습니다.](https://github.com/JSeong2024/2025-MYPAUL-PYTHONEDU/blob/main/PYTHON-2025-09/Basic/Lecture/Week-02/Week%2002_Summary_Basic.pdf)

## 2025.09.24. 수요일
- 2주차 수업이 **9월 26일** 금요일으로 변경되었습니다. (**내가 조정한거 아님**)

## 2025.09.21. 일요일
- **수업 속도**가 조금 **천천히** 진행되도록 조정할 예정입니다.
  - 코드 짤 시간을 충분히 드릴것입니다.
  - 대신 여러 난이도의 코드 문제를 제시할 것이고, 이전 단계를 빨리 마무리한 학생은 다음단계 코드를 진행할 수 있도록 할 것입니다.

- **10월8일 수요일**까지 **설문조사**를 진행해주시기 바랍니다.
  - 링크: https://docs.google.com/forms/d/e/1FAIpQLSdoYDuR8HQn5eQDHr1Xm_JAeD7qg99p10X0QY0Aay3nqGy9Vg/viewform?usp=header
  - 파이썬 초급과정을 신청한 이유, 앞으로의 진로 계획 등을 알고자합니다.
  - 학생이 원할경우 진로와 관련된 자료(프로그래밍과 관련된 내용만)를 제공할 것입니다.

- 수업 후 설문조사를 통해 수업 환경을 꾸준히 개선할 것이므로 요청사항이 있다면 언제든지 알려주시기 바랍니다. 

- **학생 개인 이메일 상태 재확인**
  - 일부 메일 주소가 잘못되었거나 휴면상태인 주소가 있는 것으로 확인되었습니다.

    **현재 사용하고 있는 메일**으로 **eins2017@naver.com**에 **학생이름**, **기초A** 라고 보내주시기 바랍니다.

## 2025.09.20. 토요일
- [1주차 내용 정리본을 업로드 하였습니다.](https://github.com/JSeong2024/2025-MYPAUL-PYTHONEDU/blob/main/PYTHON-2025-09/Basic/Lecture/Week-01/Week%2001_Summary_Basic.pdf)
- 2주차 수업은 **10월 10일** 금요일입니다. (9월 26일과 10월 3일에는 수업이 없습니다.)
